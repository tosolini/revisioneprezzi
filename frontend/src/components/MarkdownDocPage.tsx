...
// Riconosce i paragrafi introduttivi delle tabelle ("12. La Tabella D.2. reca...")
// e restituisce l'id ancora corrispondente (tabella-d2), altrimenti null.
function tabellaAnchorId(text: string): string | null {
  const m = text.match(/^\d+\.\s+La Tabella D\.?\s*([123])\b/i)
  return m ? `tabella-d${m[1]}` : null
}
...
function renderMarkdown(md: string, query = ''): string {
  ...
  function inlineFormat(text: string): string {
    ...
    // Immagini inline: ![alt](src) -> <img>. Deve precedere bold/link.
    t = t.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_m, alt: string, src: string) => {
      const safeSrc = escapeHtml(normalizeImgSrc(src)).replace(/"/g, '&quot;')
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^#{1,4}\s/.test(line)) {
      ...
      html.push(`<h${level} id="${slugify(text)}" style="margin-top:1.2em;margin-bottom:0.4em;color:var(--color-text-primary)">${renderedText}</h${level}>`)
      continue
    }
    ...
    // Riga contenente solo un'immagine: blocco figura centrato
    {
      const imgOnly = line.trim().match(/^!\[([^\]]*)\]\(([^)]+)\)\s*$/)
      if (imgOnly) {
        const src = normalizeImgSrc(imgOnly[2]).replace(/"/g, '&quot;')
        const alt = escapeHtml(imgOnly[1]).replace(/"/g, '&quot;')
        html.push(`<figure class="md-figure"><img src="${src}" alt="${alt}" loading="lazy" /></figure>`)
        continue
      }
    }
    ...
    if (/^\s*\d+\.\s/.test(line)) {
      ...
      const anchorId = tabellaAnchorId(line.trim())
      const idAttr = anchorId ? ` id="${anchorId}"` : ''
      const renderedText = query.trim() ? highlightHtmlText(inlineFormat(text), query) : inlineFormat(text)
      html.push(`<li${idAttr} style="margin:2px 0;color:var(--color-text-secondary)">${renderedText}</li>`)
      continue
    }
...
export default function MarkdownDocPage({ src, searchPlaceholder, quickNav }: { src: string; searchPlaceholder: string; quickNav?: QuickNavLink[] }) {
  ...
  const rendered = useMemo(() => {
    if (!content) return ''
    let html = renderMarkdown(content, normalizedQuery)
    if (quickNav?.length) {
      const nav = `<nav class="doc-quicknav" aria-label="Collegamenti rapidi">` +
        quickNav.map(l => `<a href="${l.href.replace(/"/g, '&quot;')}">${escapeHtml(l.label)}</a>`).join('') +
        `</nav>`
      // La nav viene inserita subito dopo il titolo iniziale (prima intestazione)
      html = html.replace(/<\/h[1-4]>/, m => `${m}\n${nav}`)
    }
    return html
  }, [content, normalizedQuery, quickNav])
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .doc-quicknav { display: flex; gap: 10px; flex-wrap: wrap; margin: 0.8em 0 1em; }
        .doc-quicknav a { display: inline-block; padding: 8px 14px; border: 1px solid var(--color-primary); border-radius: 8px; color: var(--color-primary); text-decoration: none; font-size: 13px; font-weight: 600; }
        .doc-quicknav a:hover { background: var(--color-primary); color: #fff; }
        li[id^="tabella-"] { scroll-margin-top: 70px; }
        li[id^="tabella-"]:target { background: rgba(255, 204, 0, 0.15); border-radius: 6px; }
        ...
        .search-snippet { background: var(--color-bg-offset); border: 1px solid var(--color-border-light); border-radius: 8px; padding: 10px 12px; font-size: 13px; color: var(--color-text-secondary); }
        .md-figure { margin: 1em auto; text-align: center; }
        .md-figure img, p img { max-width: 100%; height: auto; border-radius: 10px; border: 1px solid var(--color-border-light); box-shadow: 0 2px 12px var(--color-shadow); }
