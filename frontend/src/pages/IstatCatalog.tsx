...
import { DATAFLOW_BY_TYPE, EXPLORER_LABELS, EXPLORER_LEAF_URLS, sdmxTemplateFor } from "../constants/istatExplorer"
...
export default function IstatCatalog() {
  ...
  const sdmxDeepLinkOpened = useRef(false)
  const searchActive = searchQuery.trim().length > 0
  const visibleGroups = groups.filter(g => g.observation_count > 0)
  const [searchParams] = useSearchParams()
  useEffect(() => { const q = searchParams.get("q"); if (q) setSearchQuery(q); if (searchParams.get("sdmxImport") === "1" && !sdmxDeepLinkOpened.current) { sdmxDeepLinkOpened.current = true; setShowSdmx(true) } }, [searchParams])
  const parseErrorDetail = async (res: Response): Promise<string> => {
    let detail = await res.text()
    try {
      const j: unknown = JSON.parse(detail)
      ...
      <div style={{
        ...
        <div style={{ padding: "14px 16px", borderRadius: 12, border: "1px solid var(--color-border-light)", background: "var(--color-bg-muted)", display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, lineHeight: 1.5, color: "var(--color-text-secondary)" }}>
            Parti da queste 4 tabelle EsploraDati e filtra per il codice della tabella (ATECO/ECOICOP). Copia il template SDMX per evitare 422 <code style={{ fontFamily: "monospace", fontSize: 11 }}>unfiltered_dimensions</code> (IR richiede <code style={{ fontFamily: "monospace", fontSize: 11 }}>DATA_TYPE=N</code>).
          </div>
              ...
              return (
                <div key={t} style={{ padding: "10px 12px", borderRadius: 10, background: "var(--color-bg-card)", border: "1px solid var(--color-border-light)" }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap" }}>
                    <a href={leaf} target="_blank" rel="noopener noreferrer" title="Apri tabella su EsploraDati" style={{ display: "inline-block", padding: "2px 6px", borderRadius: 6, border: "1px solid var(--color-border-light)", background: "var(--color-bg-muted)", fontSize: 10, cursor: "pointer", color: "var(--color-text-primary)", textDecoration: "none", fontWeight: 600 }}>Apri EsploraDati ↗</a>
                  </div>
                  ...
                  {t === "PPI" && <div style={{ fontSize: 10, color: "var(--color-text-light)" }}>Tabella diretta: 145_360…</div>}
                  {t === "PPS" && <div style={{ fontSize: 10, color: "var(--color-text-light)" }}>Tabella diretta: 145_376… trimestrale Q</div>}
                </div>
              )
            })}
          </div>
          <div style={{ fontSize: 11, color: "var(--color-text-muted)", lineHeight: 1.4 }}>
            Suggerimento: digita un codice (es. <code>263</code>, <code>951</code>, <code>0117</code>) nella ricerca sotto — il template SDMX si aggiorna e la tabella “Risultati ricerca” filtra <code>ISTAT_*</code> localmente. Se il risultato è vuoto, usa la tabella EsploraDati corrispondente e filtra ATECO/ECOICOP lì, poi copia Query SDMX → Incolla in “Importa Query SDMX”.
          </div>
        </div>
        <input
          type="text"
