...
def list_tol(
    include_specialized: bool = True, db: Session = Depends(get_db)
...
def get_tol_detail(tol_code: str, db: Session = Depends(get_db)) -> TolMasterResponse:
    ...
    """
    tol = db.execute(select(TolMaster).where(TolMaster.code == tol_code)).scalar_one_or_none()
...
def get_tol_indices(
    tol_code: str, active_only: bool = True, db: Session = Depends(get_db)
...
def get_tol_with_indices(tol_code: str, db: Session = Depends(get_db)) -> TolWithIndicesResponse:
    """
...
def create_case_tol_assignments(
    case_id: UUID, data: TolAssignmentBulkCreate, db: Session = Depends(get_db)
) -> list[TolAssignmentResponse]:
    """
    ...
    case = db.execute(select(CaseFile).where(CaseFile.id == case_id)).scalar_one_or_none()
    ...
    existing_tols = (
        db.execute(select(TolMaster.code).where(TolMaster.code.in_(tol_codes))).scalars().all()
    )

    missing_codes = set(tol_codes) - set(existing_tols)
    if missing_codes:
        raise HTTPException(
            status_code=400, detail=f"Codici TOL non validi: {', '.join(missing_codes)}"
        )

    # Cancella assegnazioni precedenti
    db.execute(select(TolAssignment).where(TolAssignment.case_id == case_id))
    ...
    for a in data.assignments:
        assignment = TolAssignment(
            case_id=case_id, tol_code=a.tol_code, weight_percent=a.weight_percent, notes=a.notes
        )
        db.add(assignment)
...
def update_tol_assignment(
    case_id: UUID, assignment_id: UUID, data: TolAssignmentUpdate, db: Session = Depends(get_db)
) -> TolAssignmentResponse:
    """
...
def delete_tol_assignment(
    case_id: UUID, assignment_id: UUID, db: Session = Depends(get_db)
) -> None:
    """
...
def validate_tol_assignments(case_id: UUID, db: Session = Depends(get_db)) -> TolValidationResponse:
    """
    ...
    if not assignments:
        ...
        return TolValidationResponse(is_valid=False, total_weight=0.0, errors=errors)
    ...
    if len(assignments) > 1:
        if abs(total_weight - 100.0) > 0.01:
            errors.append(f"I pesi devono sommarsi a 100% (attuale: {total_weight:.2f}%)")
    ...
    return TolValidationResponse(
        is_valid=len(errors) == 0, total_weight=total_weight, errors=errors, warnings=warnings
    )
